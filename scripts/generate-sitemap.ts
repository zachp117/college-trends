 ...
 *
 * Runs after `vite build` and writes `dist/sitemap.xml` containing:
 *   - the static landing / app / about pages
 *   - one entry per currently-operating College Scorecard institution
 *     (so every /school/<slug>-<id> page is discoverable by Google)
 *
 ...
 * static-only sitemap so the build never breaks.
 */
